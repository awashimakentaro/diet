/* 【責務】
 * workout_logs の行を WorkoutLog に変換する。
 */

import type { WorkoutIntensity, WorkoutKind, WorkoutLog } from '../types';
import { normalizeWorkoutExercises } from './normalize-workout-exercises';

type WorkoutLogRow = {
  id: string;
  workout_kind?: string | null;
  menu_id: string | null;
  menu_name: string;
  exercise_name: string;
  sets: number;
  reps: number;
  weight_kg: number | string;
  exercises?: unknown;
  duration_minutes: number;
  intensity: string;
  burned_kcal: number | string;
  note: string | null;
  performed_at: string;
};

function toWorkoutIntensity(value: string): WorkoutIntensity {
  if (value === 'light' || value === 'hard') {
    return value;
  }

  return 'normal';
}

function toWorkoutKind(value: string | null | undefined): WorkoutKind {
  return value === 'other' ? 'other' : 'strength';
}

export function mapWorkoutLogRow(row: WorkoutLogRow): WorkoutLog {
  return {
    id: row.id,
    kind: toWorkoutKind(row.workout_kind),
    menuId: row.menu_id,
    name: row.menu_name,
    exercises: normalizeWorkoutExercises(row.exercises, {
      exerciseName: row.exercise_name,
      sets: row.sets,
      reps: row.reps,
      weightKg: Number(row.weight_kg),
    }),
    durationMinutes: row.duration_minutes,
    intensity: toWorkoutIntensity(row.intensity),
    estimatedBurnedKcal: Number(row.burned_kcal),
    burnedKcal: Number(row.burned_kcal),
    note: row.note ?? '',
    createdAt: row.performed_at,
    performedAt: row.performed_at,
  };
}
