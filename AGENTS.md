 ...
 */
```
- 原則として、ファイル先頭コメントは `【責務】` のみでよい。
- `使用されるエージェント / 処理フロー`、`やらないこと`、`他ファイルとの関係` などの長い定型ヘッダは不要とする。
- 下記ヘッダを欠くファイルは未完成・未レビュー・実装違反とみなす。
- 関数単位の `@returns`、`@remarks`、`呼び出し元` などの JSDoc は、特別な必要がある場合を除き不要とする。
...
- ドキュメントコメントの省略禁止。
- コメントは簡潔に保ち、原則としてファイル先頭の `【責務】` のみを出力すること。
- SRP を理由とした分割を躊躇しないこと。
- 長大な定型コメントや過剰な JSDoc を自動生成しないこと。
